/**
 * Definition: TriggerEventAddTrail
 * Hash: 473b705b
 */

#include "TriggerEventAddTrail.h"

void TriggerEventAddTrail::read(const char* base, char* &ptr) {
  char *current;
  current = ptr + 0x8;
  ...
  current = ptr + 0x7c;
  readData(&snoTrail, base, current);
  current = ptr + 0x80;
  readData(&vecScalar, base, current);
  current = ptr + 0x88;
  readData(&unk_53ae317, base, current);
  current = ptr + 0x90;
  readData(&unk_760871b, base, current);
  current = ptr + 0x98;
  readData(&unk_526c582, base, current);
  ptr += 0xa8;
}
