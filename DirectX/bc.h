...
#define NUM_PIXELS_PER_BLOCK 16
  ...
  struct BC4_UNORM
  {
    float R(std::size_t uOffset) const noexcept;
    float DecodeFromIndex(std::size_t uIndex) const noexcept;
    std::size_t GetIndex(std::size_t uOffset) const noexcept;
    void SetIndex(std::size_t uOffset, std::size_t uIndex) noexcept;
...
#pragma warning(disable : 4127)
  ...
  void OptimizeAlpha(float *pX, float *pY, const float *pPoints, uint32_t cSteps) noexcept
  {
    ...
    *pX = (fX < MIN_VALUE) ? MIN_VALUE : (fX > MAX_VALUE) ? MAX_VALUE
                                                          : fX;
    *pY = (fY < MIN_VALUE) ? MIN_VALUE : (fY > MAX_VALUE) ? MAX_VALUE
                                                          : fY;
  }
#pragma warning(pop)
  ...
  void D3DXDecodeBC6HU(XMVECTOR *pColor, const uint8_t *pBC) noexcept;
  void D3DXDecodeBC6HS(XMVECTOR *pColor, const uint8_t *pBC) noexcept;
  void D3DXDecodeBC7(XMVECTOR *pColor, const uint8_t *pBC) noexcept;
  ...
  void D3DXEncodeBC6HU(uint8_t *pBC, const XMVECTOR *pColor, uint32_t flags) noexcept;
  void D3DXEncodeBC6HS(uint8_t *pBC, const XMVECTOR *pColor, uint32_t flags) noexcept;
