...
#pragma warning(disable : 4201)
  ...
  float BC4_UNORM::R(std::size_t uOffset) const noexcept
  {
    const std::size_t uIndex = GetIndex(uOffset);
    return DecodeFromIndex(uIndex);
  }

  float BC4_UNORM::DecodeFromIndex(std::size_t uIndex) const noexcept
  {
    if (uIndex == 0)
  ...
  std::size_t BC4_UNORM::GetIndex(std::size_t uOffset) const noexcept
  {
    return static_cast<std::size_t>((data >> (3 * uOffset + 16)) & 0x07);
  }

  void BC4_UNORM::SetIndex(std::size_t uOffset, std::size_t uIndex) noexcept
  {
  ...
  // BC4S/BC5S
  float BC4_SNORM::R(std::size_t uOffset) const noexcept
  {
    const std::size_t uIndex = GetIndex(uOffset);
    return DecodeFromIndex(uIndex);
  }

  float BC4_SNORM::DecodeFromIndex(std::size_t uIndex) const noexcept
  {
  ...
  std::size_t BC4_SNORM::GetIndex(std::size_t uOffset) const noexcept
  {
    return static_cast<std::size_t>((data >> (3 * uOffset + 16)) & 0x07);
  }

  void BC4_SNORM::SetIndex(std::size_t uOffset, std::size_t uIndex) noexcept
  {
    data &= ~(uint64_t(0x07) << (3 * uOffset + 16));
    data |= (uint64_t(uIndex) << (3 * uOffset + 16));
  }
...
#pragma warning(pop)
  ...
  //-------------------------------------------------------------------------------------
  void FloatToSNorm(float fVal, int8_t *piSNorm) noexcept
  {
      ...
      const float theTexelsU[]) noexcept
  {
    ...
    {
      std::size_t uBestIndex = 0;
      float fBestDelta = 100000;
      for (std::size_t uIndex = 0; uIndex < 8; uIndex++)
      {
